...
from typing import Dict, Any, Optional, List, Tuple # List and Tuple were not used by the new method but are fine to keep

from app.config.config import Config

# Configure logging
# The basicConfig should ideally be called only once at the application entry point.
# If it's here, ensure it's not conflicting with other logging setups.
logging.basicConfig(
    level=logging.INFO,
...
class BaseExtractor(ABC):
    ...
    def extract_website_metadata(self, url: str) -> dict:
        """
        Extracts title and description metadata from a given URL.
        """
        metadata = {'title': 'Unknown', 'description': 'No description available'}
        try:
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
            # Consider using proxy if configured: self.get_random_proxy()
            proxies = {}
            if self.use_proxies:
            ...
            response = requests.get(url, headers=headers, proxies=proxies, timeout=self.browser_timeout) # Using configured timeout
            ...
            else:  # Fallback for OpenGraph description
                og_description = soup.find('meta', property='og:description')
                if og_description and og_description.get('content'):
                    metadata['description'] = og_description.get('content').strip()
            
        except requests.exceptions.Timeout:
            logger.warning(f"Timeout while fetching website {url} for metadata.")
            metadata['description'] = "Error fetching data: Timeout"
        except requests.exceptions.HTTPError as e:
            logger.warning(f"HTTP error {e.response.status_code} while fetching website {url} for metadata: {e}")
            metadata['description'] = f"Error fetching data: HTTP {e.response.status_code}"
        except requests.RequestException as e:
            logger.warning(f"Error fetching website {url} for metadata: {e}")
            metadata['description'] = f"Error fetching data: {e}"
        except Exception as e: # Catching other potential errors during parsing
            logger.warning(f"Error parsing website {url} for metadata: {e}")
            metadata['description'] = f"Error parsing data: {e}"
        return metadata
...
class HybridExtractor:
    ...
    def extract_multiple_platforms(self, urls: Dict[str, str]) -> Dict[str, Dict[str, Any]]:
        ...
        for platform, url in urls.items():
            if url:
                ...
                if platform != list(urls.keys())[-1]: # Check if it's not the last platform
    ...
    def calculate_time_based_metrics(self, historical_data: List[Dict[str, Any]], days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            time_span_weeks = time_span_seconds / (60 * 60 * 24 * 7) if time_span_seconds > 0 else 0 # Avoid division by zero
            post_frequency = len(sorted_data) / time_span_weeks if time_span_weeks > 0 else 0

            # Calculate follower growth rate
            oldest_followers = sorted_data[0].get("followers")
            newest_followers = sorted_data[-1].get("followers")
            
            growth_rate = 0 # Default to 0
            if oldest_followers is not None and newest_followers is not None and oldest_followers > 0 :
                growth_rate = ((newest_followers - oldest_followers) / oldest_followers * 100)


            # Calculate engagement trend
            engagement_rates = [data.get("engagement_rate", 0) for data in sorted_data if "engagement_rate" in data and data.get("engagement_rate") is not None]
    ...
    def _calculate_trend(self, values: List[float]) -> str:
        ...
        # Define a threshold for what's considered a significant slope
        slope_threshold = 0.05 # This can be adjusted
